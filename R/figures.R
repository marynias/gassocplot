...
##                                                                    ##
## 31/08/18     
## 12/10/18 - modify to include plotting of prob in stacked plot.
########################################################################
...
plot_assoc_stack <- function(data, corr, x.min, x.max, top.marker, type=log10p){
  ...
  if(type=="log10p"){my_ylab <- expression("-log"["10"]*paste("(",italic("p"),")"))}else{if(is.null(ylab)){my_ylab <- "Probability"}}  
  marker.plot <- plot_assoc(data, corr, x.min, x.max, ylab, type)
...
stack_assoc_plot <- function(markers, z, corr=NULL, traits, x.min=NULL, x.max=NULL, top.marker=NULL, legend=TRUE, type="log10p"){
  ...
  for(i in length(traits):1){
    ...
    marker.plot <- plot_assoc_stack(data, corr, x.min, x.max, top.marker, type)
